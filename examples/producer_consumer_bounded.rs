...
impl SystemMetrics {
    fn new(num_producers: usize, num_consumers: usize) -> Self {
        Self {
            producer_stats: (0..num_producers)
                .map(|_| ComponentStats::default())
                .collect(),
            consumer_stats: (0..num_consumers)
                .map(|_| ComponentStats::default())
                .collect(),
    ...
    fn print_summary(&self) {
        ...
        // Buffer statistics
        let (buffer_ops, buffer_failed, _buffer_time, buffer_backpressure, _) =
            self.buffer_stats.get_stats();
        ...
        println!("\nSystem Performance:");
        println!(
            "Total Produced: {} | Total Consumed: {} | Buffer Efficiency: {:.2}%",
            total_produced,
            total_consumed,
            if total_produced > 0 {
                (total_consumed as f64 / total_produced as f64) * 100.0
            } else {
                0.0
            }
        );
        ...
        println!(
            "Producer Backpressure: {} | Consumer Backpressure: {} | Total Backpressure: {}",
            total_producer_backpressure,
            total_consumer_backpressure,
...
fn consumer(
    ...
    while !metrics.is_shutdown_signaled() {
        ...
        let actual_processing_time =
            Duration::from_millis((base_processing_time.as_millis() as f64 * data_factor) as u64);
        ...
        result_data.insert(
            "original_producer".to_string(),
            data_item.producer_id.to_string(),
        );
        ...
        result_data.insert(
            "queue_time_ms".to_string(),
            queue_time.as_millis().to_string(),
        );
        result_data.insert(
            "processing_time_ms".to_string(),
            actual_processing_time.as_millis().to_string(),
        );

        // Add some computed results
        let checksum: u32 = data_item
            .data
            .iter()
            .enumerate()
...
fn result_collector(
    ...
    while let Ok(result) = result_rx.recv() {
        // Collect statistics
        let priority_count = priority_counts
            .entry(format!(
                "{:?}",
                result
                    .result_data
    ...
    if !results.is_empty() {
        ...
        println!("\nTiming Analysis:");
        println!(
            "Average Queue Time: {:.2}ms | Min: {:.2}ms | Max: {:.2}ms",
    ...
    println!(
        "✅ Result Collector completed - {} results processed",
        results.len()
    );
}
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--help" => {
                ...
                println!(
                    "  --total-items <N>                 Total items to produce [default: 1000]"
                );
    ...
    may::coroutine::scope(|scope| {
        ...
        go!(scope, move || {
            ...
            loop {
                ...
                let total_produced: u64 = metrics_clone
                    .producer_stats
                    .iter()
                    .map(|s| s.items_processed.load(Ordering::Relaxed))
                    .sum();

                let total_consumed: u64 = metrics_clone
                    .consumer_stats
                    .iter()
                    .map(|s| s.items_processed.load(Ordering::Relaxed))
                    .sum();
...
mod tests {
    ...
    fn test_small_producer_consumer() {
        ...
        let produced = metrics.producer_stats[0]
            .items_processed
            .load(Ordering::Relaxed);
        let consumed = metrics.consumer_stats[0]
            .items_processed
            .load(Ordering::Relaxed);
