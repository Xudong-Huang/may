...
mod integration_tests {
    ...
    mod safe_spawn_tests {
        ...
        fn test_different_safety_levels() {
            ...
            may::coroutine::scope(|_scope| {
                ...
                let permissive_result = permissive_handle.join().expect("Permissive coroutine panicked");
    ...
    mod tcp_networking_tests {
        ...
        fn test_tcp_echo_server_client() {
            ...
            may::coroutine::scope(|scope| {
                ...
                go!(scope, move || {
                    ...
                    client.write_all(test_data).expect("Failed to write to server");
                    
                    let mut response = vec![0; test_data.len()];
                    client.read_exact(&mut response).expect("Failed to read response");
        ...
        fn test_tcp_multiple_clients() {
            ...
            may::coroutine::scope(|scope| {
                ...
                for i in 0..client_count {
                    ...
                    go!(scope, move || {
                        ...
                        client.write_all(test_data.as_bytes()).expect("Failed to write to server");
            ...
            assert_eq!(responses_received.load(Ordering::SeqCst), client_count as u32);
        }
            ...
            may::coroutine::scope(|scope| {
                ...
                go!(scope, move || {
                    ...
                    if let Ok((len, addr)) = socket.recv_from(&mut buffer) {
                        socket.send_to(&buffer[0..len], addr).expect("Failed to echo UDP message");
                    }
                });
                ...
                go!(scope, move || {
                    ...
                    client.send_to(test_data, ("127.0.0.1", port))
                        .expect("Failed to send UDP message");
                    
                    let mut response = vec![0; test_data.len()];
                    let (len, _) = client.recv_from(&mut response)
            ...
            may::coroutine::scope(|scope| {
                ...
                for i in 0..client_count {
                    ...
                    go!(scope, move || {
                        ...
                        client.send_to(test_data.as_bytes(), ("127.0.0.1", port))
                            .expect("Failed to send UDP message");
    ...
    mod http_server_tests {
        ...
        fn test_http_server_basic_requests() {
            ...
            may::coroutine::scope(|scope| {
                ...
                go!(scope, move || {
                    ...
                    client.write_all(request.as_bytes()).expect("Failed to send HTTP request");
                    
                    let mut response = vec![0; 1024];
                    let n = client.read(&mut response).expect("Failed to read HTTP response");
        ...
        fn test_http_server_concurrent_requests() {
            ...
            assert_eq!(responses_received.load(Ordering::SeqCst), request_count as u32);
        }
    }
